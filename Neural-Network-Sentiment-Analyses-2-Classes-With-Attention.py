...
#Need several embeddings, averaged line 365
...
def add_features(df):
    ...
    #df['words_vs_caracters'] = df['num_words'] / df['total_length']
...
class Caps_Layer(nn.Module):
    def __init__(self, input_dim_capsule=gru_len * 2, num_capsule=Num_capsule, dim_capsule=Dim_capsule, \
                 routings=Routings, kernel_size=(6, 1), share_weights=True,
...
def bestThresshold(y_train,train_preds):
    ...
    delta = 0
    for tmp[0] in tqdm(np.arange(0.1, 0.501, 0.005)):
...
resultat = df_test[['qid']].copy()
resultat['prediction'] = (test_preds > delta).astype(int)
